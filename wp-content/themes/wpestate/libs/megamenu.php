   ...
   // var $tree_type = array( 'post_type', 'taxonomy', 'custom' );
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( esc_html__( '%s (Invalid)', "wpestate"), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            $title = sprintf( esc_html__('%s (Pending)', "wpestate"), $item->title );
        }
        ...
        <li id="menu-item-<?php echo esc_html($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_html($item_id); ?>">
                ...
                <label for="edit-menu-item-target-<?php echo esc_html($item_id); ?>">
                        <strong><?php esc_html_e( 'Menu Item Icon(ex: fa fa-comment)', "wpestate" ); ?></strong><br />
                ...
                <p class="field-megamenu-checkbox">
                    ...
                    <label for="edit-menu-item-megamenu-<?php echo esc_html($item_id); ?>">
                        ...
                        <strong><em><?php esc_html_e( 'Set as Mega Menu?', "wpestate" ); ?></em></strong>
                    </label>
                </p>
                 ...
                 <p class="field-megamenu-widgets description description-wide">
                    <label for="edit-menu-item-megamenu-widgetarea-<?php echo esc_html($item_id); ?>">
                        <?php esc_html_e( 'Mega Menu Widget Area', 'wpestate' ); ?>
                        <select id="edit-menu-item-megamenu-widgetarea-<?php echo esc_html($item_id); ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-megamenu-widgetarea[<?php echo esc_html($item_id); ?>]">
                            <option value="0"><?php esc_html_e( 'Select Widget Area', 'wpestate' ); ?></option>
                            <?php
                ...
                <a href="#" id="wpestate-media-upload-<?php echo esc_html($item_id); ?>" class="load_back_menu  "><?php esc_html_e( 'Upload Background Image | ', 'wpestate' ); ?></a>
              
                <a href="#" id="wpestate-media-remove-<?php echo esc_html($item_id); ?>" class="remove-megamenu-background" style="<?php echo ( trim( $item->megamenu_background ) ) ? 'display: inline;' : '';?>"><?php esc_html_e('Remove Image','wpestate');?></a>
                ...
                <p class="field-megamenu-styles description description-wide">
                    <label for="edit-menu-item-megamenu-styles-<?php echo esc_html($item_id); ?>">
                        <?php esc_html_e( 'Container Styles( *set custom styles for mega menu container only:.  Ex: background position, background repeat )', "wpestate" ); ?><br />
                ...
                <p class="field-megamenu-checkbox-border">
                    ...
                    <label for="menu-item-megamenu-border-<?php echo esc_html($item_id); ?>">
                        ...
                        <strong><em><?php esc_html_e( 'Draw border right on mega menu column?', "wpestate" ); ?></em></strong>
                    </label>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s', "wpestate"), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e('Remove', "wpestate" ); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_html($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, esc_url( admin_url( 'nav-menus.php' ) ) ) ) );
                        ?>#menu-item-settings-<?php echo esc_html($item_id); ?>"><?php esc_html_e('Cancel', "wpestate"); ?></a>
                </div>
